...
├── evaluate_rag.py           # 메인 평가 스크립트 (RAG Agent 기능)
├── evaluate_vectorstore.py   # 🆕 벡터스토어 파라미터 실험
...
| 실험 | 설명 | 테스트 변수 |
|-----|------|-----------|
| `chunk_size` | 청크 크기 비교 | 500, 1000, 1500, 2000 |
| `overlap` | 오버랩 크기 비교 | 0, 100, 200, 300 |
| `splitter_type` | 분할기 유형 비교 | recursive, token, sentence-aware |
| `full` | 전체 조합 실험 | 모든 설정 |
...
```bash
# Chunk size 실험
python evaluate_vectorstore.py --experiment chunk_size --output results/chunk_exp.json

# Splitter type 실험
python evaluate_vectorstore.py --experiment splitter_type --output results/splitter_exp.json

# 전체 실험 (모든 조합)
python evaluate_vectorstore.py --experiment full --output results/full_vectorstore.json

# 기존 결과 분석만
python evaluate_vectorstore.py --analyze-only results/chunk_exp.json --output-dir analysis/
```
...
**Chunk Size 실험:**
```
Config A:  chunk_size=500,  overlap=100  (세밀한 검색)
Config B:  chunk_size=1000, overlap=200  (Baseline)
Config C:  chunk_size=1500, overlap=300  (넓은 문맥)
Config D:  chunk_size=2000, overlap=400  (최대 문맥)
```

**Splitter Type 실험:**
```
recursive_default:   기본 RecursiveCharacterTextSplitter
recursive_sentence:  문장 단위 우선 분할 (". ", "? ", "! ")
token_based:         TokenTextSplitter (256 토큰)
recursive_paragraph: 단락 우선 분할 ("\n\n\n", "\n\n")
```
...
├── vectorstore_report.md       # 마크다운 보고서
├── vectorstore_experiment.png  # 시각화 차트
└── vectorstore_results.csv     # CSV 결과
```
